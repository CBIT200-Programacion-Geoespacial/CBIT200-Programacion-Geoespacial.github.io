...
Datos %>% as.data.frame() %>% 
  group_by(species) %>% 
  summarise(n = n(), media = mean(Measurement), maximo = max(Measurement), des_est = sd(Measurement)) %>%
  arrange(desc(media)) %>% 
  kable() %>% 
...
kk <- Datos |>
  as.data.frame() |>
  filter(species == 'Bombus occidentalis' & Measurement >0) |>
