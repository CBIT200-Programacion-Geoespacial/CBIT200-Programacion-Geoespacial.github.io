...
Usando el número de registro para _Puma concolor_ en la Base de datos GBIF podemos
contarlos:

```{r pumas-n-arg, echo=TRUE}
...
![_Octodon
degus_ (Molina, 1782)](https://upload.wikimedia.org/wikipedia/commons/4/47/Octodon_degus_-Heidelberg_Zoo%2C_Germany-8a.jpg)
...
Vemos (con: `dim(od$data)`) que la consulta nos retornó `r nrow(od$data)` registros (filas) y
...
es básicamente la misma que `od`, la anterior, sólo que se puede
manipular espacialente y tiene una columna llamada `geometry`. Esto nos permite manipular los datos como si cada fila fuera un punto a ver en un mapa pues cada registro (fila) tiene asociada una coordenada.
...
ggplot() + 
  geom_sf(data=cl,alpha=.6 ) +
  geom_sf(data = od_sf, pch=4 ) +
...
ggplot() + 
  geom_sf(data=comunas, alpha=.7) +
    geom_sf(data = od_sf, col="blue") + theme_bw() +
  ggtitle("Registros de O. degus en GBIF")
```

### Visualización interactiva

Podemos usar mapview para explorar los datos de forma interactiva, usando la columna `year` para ver como se ha muestreado a través de los años. Vamos a usar el parámetro `cex` para ajustar el tamaño del marcador (punto) que nos permite visualizar el número de registros por año. 
...
mapview(conteo,zcol='year',cex="n", alpha=.6, layer.name='Año')
```


### Serie de tiempo
veamos como ha sido el esfuerzo de muestrea a través del tiempo. Interesa saber si ha habido una tendencia en el número de registros. Para eso, graficamos la seria de tiempo y agregamos una linea de tendencia.
...
Vemos que existe una tendencia de una disminución en el número de registros, explicado por un pulso de mucho muestreo en la seguna mitad de los años 70, y que ha disminuido con el tiempo.
...
Vemos que el esfuerzo de muestro es espacialmente heterogeneo y está fuertemente determinado por el muestreo en la región de Santiago a mediados de los 70.
